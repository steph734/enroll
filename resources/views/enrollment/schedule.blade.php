                ...
                <div class="d-flex gap-2">
                    <a href="{{ route('schedule.create') }}">
                        <button class="btn btn-primary p-1"
                            ...
                            <i class="fa fa-solid fa-plus"></i> Add Schedule
                        </button>
                    </a>
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <li><a class="dropdown-item {{ !$strand ? 'active' : '' }}"
                                    href="{{ route('schedule.index', ['search' => $search]) }}">All Strands</a></li>
                            @foreach ($strands as $id => $strandName)
                            <li>
                                <a class="dropdown-item {{ $strand && str_contains(strtolower($strandName), strtolower($strand)) ? 'active' : '' }}"
                                    href="{{ route('schedule.index', ['strand' => substr(strtolower($strandName), 0, strpos(strtolower($strandName), '(') - 1), 'sort' => $sort, 'search' => $search]) }}">
                                    {{ $strandName }}
                                </a>
                            </li>
                            @endforeach
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <li><a class="dropdown-item {{ $sort === 'newest' ? 'active' : '' }}"
                                    href="{{ route('schedule.index', ['strand' => $strand, 'sort' => 'newest', 'search' => $search]) }}">Newest-Oldest</a>
                            </li>
                            <li><a class="dropdown-item {{ $sort === 'oldest' ? 'active' : '' }}"
                                    href="{{ route('schedule.index', ['strand' => $strand, 'sort' => 'oldest', 'search' => $search]) }}">Oldest-Newest</a>
                            </li>
                            <li><a class="dropdown-item {{ $sort === 'a-z' ? 'active' : '' }}"
                                    href="{{ route('schedule.index', ['strand' => $strand, 'sort' => 'a-z', 'search' => $search]) }}">A-Z</a>
                            </li>
                            <li><a class="dropdown-item {{ $sort === 'z-a' ? 'active' : '' }}"
                                    href="{{ route('schedule.index', ['strand' => $strand, 'sort' => 'z-a', 'search' => $search]) }}">Z-A</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div>
                    <form action="{{ route('schedule.index') }}" method="GET" id="searchForm">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-1">
                                    <a href="{{ route('schedule.edit', [$schedule->id, 'formtype' => 'scheduleedit']) }}"
